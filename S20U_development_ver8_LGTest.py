...
@author: hong joo LEE
BLE_devleopement_ver12 계승하였음
기기 : S20U_V8 기준
ver3 3d fft, linegraph 일정한 범위에서 무한정으로 돌도록
 3d fft 채널 바꾸기 기능 버튼 추가 진행방향 수정
 600초 창 삭제
 ble 배터리 잔량 표시 -> 5개 알고리즘 적용
 table edit 금지 모드
 5.06 csv_write 모드 추가
 ver8 LG Test 위해서 3d그래프 삭제, fft 그래프랑, raw 그래프랑 나오도록 할 예정
INA Gain          = 10
ADS1292 Gain = 12
ADS1292 Vref  = 2.42V
ADS1292 분해능 = 24bit = 2^24
신호 A x 10 x 12 < 2.42V신호 A < 2.42V/120신호 A < 20160x10^-6V신호 A < 20160uV뇌파 신호 받아들일 수 있는 최대값 20160uV
V_ref = 2.42  // ADC Reference voltageIA_G = 10     // instument amp gainADC_G=12    // adc gainADC_Bit=24  // adc resolutionUni-polar= 2bi-Polar = 1
"""
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.hBoxGraphLayout = QHBoxLayout()
        self.vBoxScanBarFFtLayout = QVBoxLayout()
        self.hBoxLayoutLabel = QHBoxLayout()
        self.vBoxLayoutPG = QVBoxLayout()

        self.vBoxLayout.addLayout(self.hBoxGraphLayout)
        self.hBoxGraphLayout.addLayout(self.vBoxScanBarFFtLayout)
        self.hBoxGraphLayout.addLayout(self.vBoxLayoutPG)
        self.vBoxLayout.addLayout(self.hBoxLayoutLabel)

        # table
        self.tableWidgetScan.setColumnCount(3)
        self.tableWidgetScan.setColumnWidth(1, 200)
        ...
        self.ax1.setRange(xRange=[-4, 0], disableAutoRange=True)
        ...
        self.ax3.setRange(xRange=[-4, 0], disableAutoRange=True)
        ...
        #self.csv_list = []
        #self.csv_fft_list = []
        self.f = open("C:/Users/KimKyoungHoon/Desktop/python_txt/data_test2.txt", 'w', newline='')
        #self.w = csv.writer(self.f)
        #self.ch1_fft_write = open("C:/Users/KimKyoungHoon/Desktop/python_txt/fft_153.csv", 'w', newline='')
        #self.ch1_fft_writer = csv.writer(self.ch1_fft_write)
        ...
        self.stop.setIcon(QIcon("./images/stopBlue2.png"))
        ...
        self.lowPass50 = QAction("S20 : 3Hz High", self)  # bandpass change
        ...
        self.bandpass50 = QAction("S20 : 4Hz High", self)
    ...
    def lowPass50Button(self):
        ...
        self.ax1.setRange(xRange=[-4, 0], yRange=[-50, 50])
        self.ax3.setRange(xRange=[-4, 0], yRange=[-50, 50])
    ...
    def read_data(self):
        while len(self.bufferLeft) > 0:
            ...
            ch1_int = ch1_int * self.max_uv / 1000
            ...
            ch2_int = ch2_int * self.max_uv / 1000
    ...
    def print_graph(self):
        ...
        if self.notchSelect == notchFilterSelect.none:
            ...
            filtering_ch1 = nf.notch_filter(notch_ch1, self.notchCutOff, self.samplingRate, self.notchQualityFactor)
            filtering_ch2 = nf.notch_filter(notch_ch2, self.notchCutOff, self.samplingRate, self.notchQualityFactor)
        filtering_ch1 = lf.butter_lowpass_filter(filtering_ch1, 30, self.samplingRate, 5)
        filtering_ch2 = lf.butter_lowpass_filter(filtering_ch2, 30, self.samplingRate, 5)
        if self.lowPassSelect == lowPassFilterSelect.none:
            pass
        elif self.lowPassSelect == lowPassFilterSelect.lpf50:
            filtering_ch1 = hf.butter_highpass_filter(filtering_ch1, 3, self.samplingRate, 5)
            filtering_ch2 = hf.butter_highpass_filter(filtering_ch2, 3, self.samplingRate, 5)
    ...
    def printFFTGraph(self):
        ...
        ch2_raw_data = ch2_raw_data * self.windowed
